...
class ImmutableProxy(Proxy):
	...
	def __iadd__(self, other):
		raise ImmutableError(('object "{0:}" is frozen; it cannot be changed in place; ' +
...
class ListProxy(ImmutableProxy):
	def append(self, p_object):
		raise ImmutableError('list "{0:}" is frozen; it cannot be changed'.format(self))
...
class DictProxy(ImmutableProxy):
	def clear(self):
		raise ImmutableError('dictionary "{0:}" is frozen; it cannot be changed'.format(self))
...
class SetProxy(ImmutableProxy):
	def add(self, *args, **kwargs):
		raise ImmutableError('set "{0:}" is frozen; it cannot be changed'.format(self))
